...
def test(loader, model, opt, cap_vocab, cms_vocab):
    ...
    for batch_id, raw_data in enumerate(loader):
        ...
        for iterate_id in range(len(raw_data)):
            ...
            #增加need，react
                ...
                cms_list = raw_data[iterate_id][4]
            else:
                cms_list = raw_data[iterate_id][5]
            print("cuda:",opt['cuda'])
            if opt['cuda']:
    ...
    with open("{}.json".format(opt['load_checkpoint'].split('/')[-1].split('.')[0]+'_'+opt['cms']), "w" , newline="\n") as f:
    ...
    print('C, R, B:', avg_cider_score, avg_rouge_score, avg_bleu_score)
    return cms

def main(opt):
    print('\nvideo:',opt['video_to_test'])
    print('\n')
    ...
    #增加need,react
        ...
        cms_text_length = opt['need_max_len']
    else:
        cms_text_length = opt['react_max_len']

    cms_int_text_length = opt['int_max_len']
    cms_eff_text_length = opt['eff_max_len']
    cms_att_text_length = opt['att_max_len']
    cms_need_text_length = opt['need_max_len']
    cms_react_text_length = opt['react_max_len']
    ...
    model = Model(
        ...
        #增加need,react
        cms_max_seq_need=cms_need_text_length,
        cms_max_seq_react=cms_react_text_length,
    ...
    print(params)
    cms = test(dataloader, model, opt, dataset.get_cap_vocab(), dataset.get_cms_vocab())
    return cms
