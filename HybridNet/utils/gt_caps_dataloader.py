...
class VideoDataset_mix40(Dataset):
    ...
    def __init__(self, opt, mode='train'):
        ...
        #test 1 video
        video_to_test = opt['video_to_test']
        if video_to_test>=0:
            print("video_to_test:",type(video_to_test),video_to_test)
            print('\n')
            self.splits['test']=[video_to_test]
    ...
    def __getitem__(self, ix):
        ...
        int_list, eff_list, att_list, need_list, react_list = [], [], [], [], []

        # Load all num_cap gt captions，对于每个视频，遍历所有的gt-caption
        for cap_ix in range(num_cap):
            ...
            #增加need,react
            intentions, effects, attributes, needs, reacts = raw_data[cap_ix]['intention'], raw_data[cap_ix]['effect'], \
                                              raw_data[cap_ix]['attribute'], raw_data[cap_ix]['need'], raw_data[cap_ix]['react']

            # Concatenate all CMS，连接所有的commonsense
            # 增加need,react
            int_str, att_str, eff_str, need_str, react_str = '', '', '', '',''
            for ints, eff, att, need, react in zip(intentions, effects, attributes, needs, reacts):
                int_str += ';' + ints[0]
                eff_str += ';' + eff[0]
                att_str += ';' + att[0]
                need_str += ';' + need[0]
        ...
        print("i3d feats:\n")
        i3d_path=os.path.join('data/i3d_features', 'msr_vtt-I3D-RGBFeatures-video%i.npy' % ix)
        if os.path.exists(i3d_path):
            ...
            i3d = np.zeros((10,1024))
        print("i3d shape",i3d.shape)
        print("\n")
        ...
        print("audio feats:\n")
        audio_path = os.path.join('data/audio_features', 'video%i.mp3.soundnet.h5' % ix)
        ...
        #增加need，react
        return data, int_list, eff_list, att_list, need_list, react_list
