...
class HybirdNet(nn.Module):
    ...
    def __init__(#增加need，react
            self,
            n_cap_vocab, n_cms_vocab, cap_max_seq, cms_max_seq_int, cms_max_seq_eff, cms_max_seq_att,cms_max_seq_need,
            cms_max_seq_react,vis_emb=2048, i3d_emb=1024, audio_emb=1024, d_word_vec=512, d_model=512, d_inner=2048,
        ...
        self.cms_decoder_att = Decoder(
            ...
            dropout=dropout)
        #增加need，react
        self.cms_decoder_need = Decoder(
            n_tgt_vocab=n_cms_vocab, len_max_seq=cms_max_seq_need,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            dropout=dropout)
        self.cms_decoder_react = Decoder(
            n_tgt_vocab=n_cms_vocab, len_max_seq=cms_max_seq_react,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            dropout=dropout)
        ...
        if tgt_emb_prj_weight_sharing:
            ...
            #增加need，react?
            self.cms_decoder_need.tgt_word_emb.weight = self.cms_decoder_react.tgt_word_emb.weight
            self.cms_decoder_att.tgt_word_emb.weight = self.cms_decoder_need.tgt_word_emb.weight
    ...
    def forward(self, vis_feat, i3d, aud, tgt_seq, tgt_pos,
                cms_seq_int, cms_pos_int, cms_seq_eff, cms_pos_eff, cms_seq_att, cms_pos_att,cms_seq_need, cms_pos_need,cms_seq_react, cms_pos_react):
        ...
        #增加need，react
        cms_seq_need, cms_pos_need = cms_seq_need[:, :-1], cms_pos_need[:, :-1]
        cms_seq_react, cms_pos_react = cms_seq_react[:, :-1], cms_pos_react[:, :-1]
        ...
        #增加need，react
        cms_dec_output_need, *_ = self.cms_decoder_need(cms_seq_need, cms_pos_need, cat_output, cat_output)
        cms_dec_output_react, *_ = self.cms_decoder_react(cms_seq_react, cms_pos_react, cat_output, cat_output)
        ...
        #增加need,react
        cms_logit_need = self.cms_word_prj(cms_dec_output_need) * self.x_logit_scale
        cms_logit_react = self.cms_word_prj(cms_dec_output_react) * self.x_logit_scale

        #增加need，react
        return seq_logit.view(-1, seq_logit.size(2)), cms_logit_int.view(-1, cms_logit_int.size(2)), cms_logit_eff.view(
            -1, cms_logit_eff.size(2)), cms_logit_att.view(-1, cms_logit_att.size(2)), cms_logit_need.view(-1, cms_logit_need.size(2)), cms_logit_react.view(-1, cms_logit_react.size(2))
