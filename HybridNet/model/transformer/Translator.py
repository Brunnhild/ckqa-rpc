...
def translate_batch_cap2cms(model, src_emb, cap_label, i3d, audio,opt):
        ...
        def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm):
            if mode == 'cap':
                ...
                if opt['cms'] == 'int':
                    ...
                    dec_output, *_ = model.cms_decoder_need(dec_seq, dec_pos, src_seq, enc_output)
                else:
                    dec_output, *_ = model.cms_decoder_react(dec_seq, dec_pos, src_seq, enc_output)
                dec_output = dec_output[:, -1, :]  # Pick the last step: (bh * bm) * d_h
    ...
    with torch.no_grad():
        ...
        if opt['cms'] == 'int':
            ...
            len_dec_seq_total = opt['need_max_len']
        else:
            len_dec_seq_total = opt['react_max_len']
